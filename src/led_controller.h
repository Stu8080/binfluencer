...
// Pulse the middle 3 alert LEDs once at 50% white to signal a schedule check.
void heartbeat() {
    static const uint8_t start = (LED_COUNT_ALERT - 3) / 2;
    fill_solid(ledsAlert + start, 3, CRGB(128, 128, 128));
    FastLED.show();
    delay(300);
    fill_solid(ledsAlert + start, 3, CRGB::Black);
    FastLED.show();
}
